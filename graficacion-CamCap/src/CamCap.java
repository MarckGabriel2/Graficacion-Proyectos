...
public class CamCap extends javax.swing.JFrame {
    ...
    CascadeClassifier CajaDetector;
    ...
    class DaemonThread implements Runnable {
        ...
        public void run() {
            ...
            CajaDetector = new CascadeClassifier(base_path + cajaFile); // Se crea un objeto CascadeClassifier que reconocera caras
            CajaDetections = new MatOfRect(); // Se inicializa el objeto donde se guardaran las caras detectadas
                ...
                while (runnable) {
                    if (webSource.grab()) {
                        try {
                            ...
                            CajaDetector.detectMultiScale(frame, CajaDetections); // Se buscan las caras dentro de la imagen y se guardan en faceDetections
                            
                            int i= 0;
                            
                            for (Rect rect : CajaDetections.toArray()) {
                                ...
                                caja = frame.submat(CajaDetections.toArray()[i]);
                                
                                
                                CajaDetector.detectMultiScale(caja, CajaDetections);

                                if (CajaDetections.toArray().length != 0) {

                                    System.out.println("Cara con " + CajaDetections.toArray().length + " ojos");

                                    for (int j = 0; j < CajaDetections.toArray().length; j++) {
                                        
                                        Rect rect2 = CajaDetections.toArray()[j];

                                        Imgproc.rectangle(frame, new Point(rect2.x, rect2.y), new Point(rect2.x + rect2.width, rect2.y + rect2.height),
                                        new Scalar(0, 255, 0));
                                        
                                        Mat eye = cara.submat(CajaDetections.toArray()[j]);
                            ...
                            System.out.println("Si jala el proyecto :D");
                        }
                    }
                }
            }
